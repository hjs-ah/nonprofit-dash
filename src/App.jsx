...
const PHASES = [
  {
    id: 1, title: "Foundation & Governance",
    tasks: [
  ...
  {
    id: 2, title: "Legal Incorporation",
    tasks: [
  ...
  {
    id: 3, title: "IRS 501(c)(3) Application",
    tasks: [
    ...
    links: [
      ...
      { l: "Submit on Pay.gov", u: "https://www.pay.gov" },
    ]
  },
  {
    id: 4, title: "State & Local Compliance",
    tasks: [
  ...
  {
    id: 5, title: "Banking & Financials",
    tasks: [
      { t: "Open a dedicated nonprofit bank account", n: "Bring EIN, Articles of Incorporation, and bylaws" },
      { t: "Set up accounting software", n: "QuickBooks Nonprofit, Wave, or Aplos are common choices" },
...
const CONSIDERATIONS = [
  { cat: "Legal", items: [
    "Ensure Articles of Incorporation include IRS-required language for dissolution and charitable purpose — missing these clauses will cause your 501(c)(3) application to be rejected.",
    "State law governs your nonprofit corporation. Federal law (IRS) governs your tax-exempt status. Both must be maintained independently.",
    "Your board must include at least 3 unrelated individuals. IRS may scrutinize boards where members are related by blood, marriage, or business interest.",
    "Private inurement prohibition: no part of net earnings may benefit any private individual. This includes excessive compensation to founders.",
  ]},
  { cat: "Financial", items: [
    "File Form 990 every year, even if you have zero revenue. Three consecutive missed filings result in automatic revocation of tax-exempt status.",
    "Restricted vs. unrestricted funds: if donors earmark gifts for specific purposes, those funds are legally restricted and must be used only for that purpose.",
    "Keep personal and organizational finances completely separate from day one. Co-mingling is a red flag during audits.",
    "Charitable deduction substantiation: donors giving $250+ must receive written acknowledgment. Verbal thank-yous don't count for their tax deduction.",
  ]},
  { cat: "Governance", items: [
    "Conflict of interest policies should be adopted at your founding meeting and re-signed annually by all board members and key staff.",
    "Board members have three legal duties: Duty of Care, Duty of Loyalty, and Duty of Obedience (comply with laws and mission).",
    "Executive compensation should be set by the board, not by the executive. Document the process using comparability data.",
    "Meeting minutes are a legal record. They should capture decisions, vote outcomes, and attendance — not a full transcript of discussion.",
  ]},
  { cat: "Fundraising", items: [
    "Most states require charitable solicitation registration before you solicit donations — even online. Fines for non-compliance can be significant.",
    "Grant compliance: foundation grants often come with reporting requirements and restrictions. Track grant terms carefully.",
    "Quid pro quo contributions (e.g., gala tickets) require you to disclose the fair market value of goods/services received.",
    "Recurring donors need to be told upfront how to cancel. FTC regulations on automatic payments apply to nonprofits.",
  ]},
  { cat: "Program", items: [
    "Your programs must align with the charitable purpose described in your 1023 application. Significant program changes should be reported to the IRS.",
    "Document program outcomes from day one — funders will ask for impact data, and retroactive data collection is unreliable.",
    "Volunteer hour tracking can be valuable for matching grant requirements and in-kind contribution documentation.",
    "If you hire paid staff, you're now subject to employment law: payroll taxes, workers' comp, and anti-discrimination requirements apply.",
  ]},
  { cat: "Technology", items: [
    "Website accessibility (WCAG 2.1 AA) is increasingly expected by major funders and may be legally required depending on your programs.",
    "Data privacy: if you collect donor or client data, create a privacy policy and understand state privacy laws (e.g., CCPA).",
    "Email marketing: CAN-SPAM and CASL apply. Always include unsubscribe options and your physical mailing address.",
    "Cybersecurity is a governance issue — boards should adopt a basic data security policy, especially if you handle sensitive client information.",
  ]},
];
...
const METRICS = [
  "People served (unduplicated)", "Volunteer hours contributed",
  "Programs offered & participants", "Geographic reach (zip codes, counties)",
  "Grants received & grant-to-program ratio", "Donor retention rate year-over-year",
  "Program cost per person served", "Staff and board diversity metrics",
];

const FINANCIALS = {
  budget: [
    { category: "Personnel (salaries, benefits)", amount: 120000 },
    { category: "Program delivery costs", amount: 85000 },
    { category: "Occupancy (rent, utilities)", amount: 24000 },
    { category: "Technology & software", amount: 8500 },
    { category: "Marketing & communications", amount: 12000 },
    { category: "Professional services (legal, CPA)", amount: 15000 },
    { category: "Insurance", amount: 6500 },
    { category: "Miscellaneous / contingency", amount: 9000 },
  ],
  revenue: [
    { category: "Individual donations", amount: 95000 },
    { category: "Foundation grants", amount: 110000 },
    { category: "Government grants", amount: 45000 },
    { category: "Events & fundraising", amount: 28000 },
    { category: "Earned income / fees", amount: 12000 },
  ],
  timeline: [
    { month: "Month 1–2", milestone: "Open bank account, set up accounting software, hire bookkeeper" },
    { month: "Month 3", milestone: "Finalize Year 1 budget, board adopts financial controls policy" },
    { month: "Month 4", milestone: "Submit first grant applications, launch initial fundraising campaign" },
    { month: "Month 6", milestone: "Mid-year financial review with board treasurer, update projections" },
    { month: "Month 9", milestone: "Begin Year 2 budget planning, assess reserve fund status" },
    { month: "Month 12", milestone: "Year-end close, prepare financial statements for Form 990" },
    { month: "Month 14–15", milestone: "File Form 990 (due 4.5 months after fiscal year end)" },
    { month: "Ongoing", milestone: "Monthly reconciliation, quarterly treasurer reports to board" },
  ],
  documents: [
    { name: "Form 1023 / 1023-EZ", desc: "IRS application for tax-exempt status", status: "Required" },
    { name: "Form 990 (annual)", desc: "Annual information return; public record", status: "Required" },
    { name: "Audited Financial Statements", desc: "Required by many funders at $500K+ revenue", status: "Recommended" },
    { name: "IRS Form W-9", desc: "Provided to grantors; confirms EIN and exempt status", status: "Required" },
    { name: "Gift Acknowledgment Letters", desc: "Written receipt for donations $250+", status: "Required" },
    { name: "Chart of Accounts", desc: "Foundation of your fund accounting system", status: "Required" },
    { name: "Financial Controls Policy", desc: "Documents dual-signature rules, expense approvals", status: "Required" },
    { name: "Investment Policy Statement", desc: "Guides management of reserve/endowment funds", status: "Recommended" },
    { name: "State Charitable Registration", desc: "Annual registration to solicit donations in each state", status: "Required" },
    { name: "1099-NEC Forms", desc: "Issued to contractors paid $600+ in a year", status: "Required" },
  ],
};

const SAMPLE_BOARD = [
  { name: "Dr. Amara Collins", role: "Board Chair", bio: "25 years in nonprofit leadership and community development.", initials: "AC", hue: "#2563eb" },
  { name: "Marcus Webb", role: "Treasurer", bio: "CPA with expertise in nonprofit fund accounting and audits.", initials: "MW", hue: "#16a34a" },
  { name: "Priya Nair", role: "Secretary", bio: "Attorney specializing in nonprofit and tax-exempt law.", initials: "PN", hue: "#7c3aed" },
  { name: "Jerome Okafor", role: "Director", bio: "Community organizer and former city council member.", initials: "JO", hue: "#dc2626" },
  { name: "Sofia Reyes", role: "Director", bio: "Marketing director with pro-bono nonprofit consulting experience.", initials: "SR", hue: "#b45309" },
];

const SAMPLE_ASSETS = [
  { title: "IRS Form 1023-EZ", url: "https://www.irs.gov/forms-pubs/about-form-1023-ez", category: "Government", desc: "Short-form application for 501(c)(3) status" },
  { title: "IRS EIN Online Application", url: "https://www.irs.gov/businesses/small-businesses-self-employed/apply-for-an-employer-identification-number-ein-online", category: "Government", desc: "Free EIN registration — results immediately" },
  { title: "Multi-State Filing Project", url: "https://www.multistatefiling.org/", category: "Legal", desc: "Unified charitable solicitation registration across states" },
  { title: "Google for Nonprofits", url: "https://www.google.com/nonprofits/", category: "Tools & Software", desc: "Free Google Workspace for verified 501(c)(3) organizations" },
  { title: "Council of Nonprofits", url: "https://www.councilofnonprofits.org/", category: "Reference", desc: "Templates, compliance calendar, and nonprofit guides" },
];

const SAMPLE_WEEKS = [
  {
    label: "Week of March 17, 2026", phase: "Foundation & Governance",
    summary: "Completed initial governance setup and began Articles of Incorporation preparation.",
    done: "• Finalized board member list (5 confirmed)\n• Adopted conflict of interest policy template\n• Confirmed organization name availability with state SCC",
    wip: "• Attorney reviewing bylaws draft\n• Preparing Articles of Incorporation language",
    blockers: "• Awaiting attorney final review — est. 5 business days",
    next: "• File Articles of Incorporation once attorney approves\n• Schedule inaugural board meeting for April 3",
  },
];
...
// ─────────────────────────────────────────────
const TAG_LIGHT = {
  Legal: { bg: "#dbeafe", text: "#1d4ed8" }, Financial: { bg: "#dcfce7", text: "#15803d" },
  Government: { bg: "#fee2e2", text: "#b91c1c" }, "Tools & Software": { bg: "#ede9fe", text: "#6d28d9" },
  Templates: { bg: "#fef9c3", text: "#92400e" }, Reference: { bg: "#f3f4f6", text: "#374151" },
  Other: { bg: "#f3f4f6", text: "#374151" }, Required: { bg: "#fee2e2", text: "#b91c1c" },
  Recommended: { bg: "#dcfce7", text: "#15803d" },
  "Foundation & Governance": { bg: "#dbeafe", text: "#1d4ed8" },
  "Legal Incorporation": { bg: "#fef9c3", text: "#92400e" },
  "IRS Application": { bg: "#fee2e2", text: "#b91c1c" },
  "State Compliance": { bg: "#dcfce7", text: "#15803d" },
  "Banking & Financials": { bg: "#ede9fe", text: "#6d28d9" },
  Operations: { bg: "#f3f4f6", text: "#374151" },
  "Ongoing Compliance": { bg: "#f0fdf4", text: "#166534" },
  Multiple: { bg: "#fce7f3", text: "#9d174d" },
};
const TAG_DARK = {
  Legal: { bg: "rgba(37,99,235,0.15)", text: "#93c5fd" }, Financial: { bg: "rgba(22,163,74,0.15)", text: "#86efac" },
  Government: { bg: "rgba(220,38,38,0.15)", text: "#fca5a5" }, "Tools & Software": { bg: "rgba(124,58,237,0.15)", text: "#c4b5fd" },
  Templates: { bg: "rgba(180,83,9,0.15)", text: "#fcd34d" }, Reference: { bg: "rgba(255,255,255,0.08)", text: "rgba(255,255,255,0.5)" },
  Other: { bg: "rgba(255,255,255,0.08)", text: "rgba(255,255,255,0.5)" }, Required: { bg: "rgba(220,38,38,0.15)", text: "#fca5a5" },
  Recommended: { bg: "rgba(22,163,74,0.15)", text: "#86efac" },
  "Foundation & Governance": { bg: "rgba(37,99,235,0.15)", text: "#93c5fd" },
  "Legal Incorporation": { bg: "rgba(180,83,9,0.15)", text: "#fcd34d" },
  "IRS Application": { bg: "rgba(220,38,38,0.15)", text: "#fca5a5" },
  "State Compliance": { bg: "rgba(22,163,74,0.15)", text: "#86efac" },
  "Banking & Financials": { bg: "rgba(124,58,237,0.15)", text: "#c4b5fd" },
  Operations: { bg: "rgba(255,255,255,0.08)", text: "rgba(255,255,255,0.5)" },
  "Ongoing Compliance": { bg: "rgba(22,163,74,0.12)", text: "#86efac" },
  Multiple: { bg: "rgba(157,23,77,0.15)", text: "#f9a8d4" },
};

function buildTokens(dark) {
  return dark ? {
    bg: "#0f0f0d", surface: "#18181b", surface2: "#1e1e22",
    ...
    progTrack: "rgba(255,255,255,0.08)", progFill: "#6ea8fe",
    sidebarBg: "#111113", inputBg: "rgba(255,255,255,0.04)",
    bottomNav: "#111113", bottomNavBorder: "rgba(255,255,255,0.08)",
  } : {
    ...
    green: "#16a34a", greenBg: "rgba(22,163,74,0.08)", red: "#dc2626", redBg: "rgba(220,38,38,0.08)",
    progTrack: "rgba(0,0,0,0.08)", progFill: "#2563eb",
    sidebarBg: "#ffffff", inputBg: "#f1f0ec",
    bottomNav: "#ffffff", bottomNavBorder: "rgba(0,0,0,0.08)",
  };
}
...
export default function App() {
  ...
  const [boardMembers, setBoardMembers] = useState(SAMPLE_BOARD);
  ...
  const [weekEntries, setWeekEntries] = useState(SAMPLE_WEEKS);
  ...
  const boardFileRef = useRef(); const pdfRef = useRef(); const [pendingBoardIdx, setPendingBoardIdx] = useState(null);
  ...
  const toggleCheck = (pid, ti) => setChecks(prev => ({ ...prev, [`${pid}-${ti}`]: !prev[`${pid}-${ti}`] }));
  const handleBoardPhoto = useCallback((idx, file) => {
    if (!file) return;
    setBoardMembers(prev => prev.map((m, i) => i === idx ? { ...m, photo: URL.createObjectURL(file) } : m));
  }, []);
  const addAsset = () => {
    if (!aTitle.trim() || !aUrl.trim()) return;
    setAssets(prev => [{ title: aTitle.trim(), url: aUrl.trim(), category: aCat, desc: aDesc.trim() }, ...prev]);
    setATitle(""); setAUrl(""); setADesc(""); setACat("Reference");
  };
  const addWeekEntry = () => {
    if (!wLabel.trim()) return;
    setWeekEntries(prev => [{ label: wLabel, phase: wPhase, summary: wSummary, done: wDone, wip: wWip, blockers: wBlockers, next: wNext }, ...prev]);
    setExpandedWeek(0);
  ...
  const ghostBtn = { padding: "8px 14px", borderRadius: 7, border: `1px solid ${T.border2}`, background: T.surface2, color: T.text, fontSize: 13, cursor: "pointer", fontFamily: "'DM Sans',sans-serif", fontWeight: 400 };
  ...
  const Sidebar = () => (
    <aside style={{ width: 210, background: T.sidebarBg, borderRight: `1px solid ${T.border}`, display: "flex", flexDirection: "column", flexShrink: 0, height: "100vh", position: "sticky", top: 0 }}>
      <div style={{ padding: "20px 16px 13px", borderBottom: `1px solid ${T.border}` }}>
        ...
        <div style={{ fontSize: 11, color: T.muted, marginTop: 2 }}>Formation & Governance</div>
      </div>
      <nav style={{ flex: 1, padding: "7px 7px", overflowY: "auto" }}>
        {NAV_ITEMS.map(item => (
          <button key={item.id} onClick={() => setActiveNav(item.id)} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "8px 10px", borderRadius: 7, border: "none", cursor: "pointer", background: activeNav === item.id ? T.accentBg : "transparent", color: activeNav === item.id ? T.accent : T.muted, fontSize: 13, fontFamily: "'DM Sans',sans-serif", fontWeight: activeNav === item.id ? 500 : 400, borderLeft: `2px solid ${activeNav === item.id ? T.accent : "transparent"}`, marginBottom: 1, textAlign: "left", transition: "all 0.1s" }}>
            <span style={{ fontSize: 11, opacity: 0.7, flexShrink: 0 }}>→</span>{item.label}
          </button>
        ))}
      </nav>
  ...
  // ─── BOTTOM NAV (mobile) — show first 5 + "More" if needed
  const BottomNav = () => {
    const visible = NAV_ITEMS.slice(0, 5);
    return (
      <nav style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: T.bottomNav, borderTop: `1px solid ${T.bottomNavBorder}`, display: "flex", zIndex: 100, paddingBottom: "env(safe-area-inset-bottom)" }}>
        {visible.map(item => (
          <button key={item.id} onClick={() => setActiveNav(item.id)} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, padding: "9px 4px 8px", border: "none", cursor: "pointer", background: "transparent", color: activeNav === item.id ? T.accent : T.muted, fontFamily: "'DM Sans',sans-serif", minWidth: 0 }}>
            <span style={{ fontSize: 16, lineHeight: 1 }}>→</span>
            <span style={{ fontSize: 10, fontWeight: activeNav === item.id ? 600 : 400, letterSpacing: "0.01em", whiteSpace: "nowrap" }}>{item.shortLabel}</span>
          </button>
        ))}
        {/* More menu for items 5-8 */}
  ...
  const MoreMenu = () => {
    ...
    const anyActive = overflow.some(i => i.id === activeNav);
    return (
      <>
        {moreOpen && (
          <div style={{ position: "fixed", bottom: 56, right: 0, background: T.surface, border: `1px solid ${T.border}`, borderRadius: 10, padding: "6px 0", minWidth: 160, zIndex: 200, boxShadow: "0 -4px 20px rgba(0,0,0,0.12)" }}>
            {overflow.map(item => (
        ...
        <button onClick={() => setMoreOpen(o => !o)} style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 3, padding: "9px 4px 8px", border: "none", cursor: "pointer", background: "transparent", color: anyActive ? T.accent : T.muted, fontFamily: "'DM Sans',sans-serif" }}>
          <span style={{ fontSize: 16, lineHeight: 1, letterSpacing: "0.08em" }}>···</span>
          <span style={{ fontSize: 10, fontWeight: anyActive ? 600 : 400 }}>More</span>
        </button>
      </>
    );
  };
  ...
  const PageOverview = () => (
    <div>
      <PageHead title="Dashboard" sub="Your nonprofit formation hub — track progress across every phase" />
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: 10, marginBottom: 18 }}>
        {[{ label: "Phases complete", val: `${phasesDone}/7` }, { label: "Steps done", val: `${doneTasks}/${totalTasks}` }, { label: "Progress", val: `${pct}%` }, { label: "Board members", val: boardMembers.length }].map(c => (
      ...
      <div style={card()}>
        <span style={lbl}>Organization profile</span>
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 10 }}>
          {[["Organization name", "e.g. Verity Foundation"], ["State of incorporation", "e.g. Virginia"], ["EIN (once assigned)", "xx-xxxxxxx"], ["Fiscal year end", "e.g. December 31"]].map(([l, ph]) => (
            <div key={l}><span style={lbl}>{l}</span><input style={inp()} type="text" placeholder={ph} /></div>
          ))}
          <div style={{ gridColumn: isMobile ? "1" : "1/-1" }}><span style={lbl}>Vision statement</span><textarea style={ta()} placeholder="The long-term future your organization aspires to create…" /></div>
          <div style={{ gridColumn: isMobile ? "1" : "1/-1" }}><span style={lbl}>Mission statement</span><textarea style={ta()} placeholder="What your organization does, for whom, and why…" /></div>
        </div>
      </div>
      <div style={card()}>
        ...
        {PHASES.map(ph => {
          ...
          return (
            <div key={ph.id} style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
              ...
              <span style={{ flex: 1, fontSize: 13, color: T.text, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: isMobile ? "nowrap" : "normal" }}>{ph.title}</span>
  ...
  const PageChecklist = () => (
    <div>
      <PageHead title="Formation Checklist" sub="Seven phases from idea to IRS-recognized 501(c)(3) status" />
      {PHASES.map(ph => {
        const done = ph.tasks.filter((_, ti) => checks[`${ph.id}-${ti}`]).length;
        const complete = done === ph.tasks.length;
        return (
          <Accord key={ph.id} open={expandedPhase === ph.id} onToggle={() => setExpandedPhase(expandedPhase === ph.id ? null : ph.id)} title={ph.title} badge={`${done}/${ph.tasks.length}`} badgeDone={complete}>
  ...
  const PageBoard = () => (
    <div>
      <PageHead title="Board of Directors" sub="Tap + on any card to upload a photo; use the add card to add new members" />
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(auto-fill,minmax(176px,1fr))", gap: 12 }}>
        {boardMembers.map((m, i) => (
          <div key={i} style={card({ textAlign: "center", padding: "18px 14px", marginBottom: 0 })}>
            <div style={{ position: "relative", display: "inline-block", marginBottom: 10 }}>
              {m.photo
                ? <img src={m.photo} alt={m.name} style={{ width: 68, height: 68, borderRadius: "50%", objectFit: "cover", border: `2px solid ${T.border}` }} />
                : <div style={{ width: 68, height: 68, borderRadius: "50%", background: m.hue + "18", border: `2px solid ${m.hue}33`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 17, fontWeight: 600, color: m.hue }}>{m.initials}</div>
              }
              <button onClick={() => { setPendingBoardIdx(i); boardFileRef.current.click(); }} style={{ position: "absolute", bottom: 0, right: 0, width: 22, height: 22, borderRadius: "50%", background: T.accent, border: "none", cursor: "pointer", color: "#fff", fontSize: 13, display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
            </div>
            <div style={{ fontSize: isMobile ? 13 : 14, fontWeight: 600, color: T.text, marginBottom: 3, lineHeight: 1.3 }}>{m.name}</div>
            <div style={{ fontSize: 10, color: T.accent, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 7, fontWeight: 500 }}>{m.role}</div>
            <div style={{ fontSize: 11.5, color: T.muted, lineHeight: 1.6 }}>{m.bio}</div>
          </div>
        ))}
        <div onClick={() => {
          const name = prompt("Board member name:"); if (!name) return;
          const role = prompt("Role (e.g. Director):") || "Director";
          const bio = prompt("Brief bio:") || "";
          const hues = ["#2563eb", "#16a34a", "#7c3aed", "#dc2626", "#b45309"];
          setBoardMembers(prev => [...prev, { name, role, bio, initials: name.split(" ").map(w => w[0]).join("").slice(0, 2).toUpperCase(), hue: hues[prev.length % hues.length] }]);
        }} style={{ background: "transparent", border: `1px dashed ${T.border2}`, borderRadius: 12, padding: "18px 14px", textAlign: "center", cursor: "pointer", color: T.muted, minHeight: 130, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 6, fontSize: 13 }}>
          <span style={{ fontSize: 20, opacity: 0.4, fontWeight: 300 }}>+</span>
          <span>Add member</span>
        </div>
      </div>
      <input ref={boardFileRef} type="file" accept="image/*" style={{ display: "none" }} onChange={e => { handleBoardPhoto(pendingBoardIdx, e.target.files?.[0]); e.target.value = ""; }} />
    </div>
  );
  ...
  const PageAssets = () => (
    <div>
      <PageHead title="Assets & Links" sub="Save links, resources, and documents with titles and categories" />
      <div style={card()}>
        ...
        <button onClick={addAsset} style={primaryBtn}>→ Add asset</button>
      </div>
      ...
      {assets.map((asset, i) => (
        <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "11px 14px", background: T.surface, border: `1px solid ${T.border}`, borderRadius: 9, marginBottom: 6 }}>
          ...
          <button onClick={() => setAssets(prev => prev.filter((_, idx) => idx !== i))} style={{ background: "none", border: "none", cursor: "pointer", color: T.muted2, fontSize: 18, padding: "0 2px", lineHeight: 1, flexShrink: 0 }}>×</button>
        </div>
      ))}
    </div>
  );
  ...
  const PageWeekly = () => (
    <div>
      <PageHead title="Weekly Activity Log" sub="Summarize actions and progress each week — like release notes" />
      <div style={card()}>
        ...
        <button onClick={addWeekEntry} style={primaryBtn}>→ Log this week</button>
      </div>
        ...
        return (
          <div key={i} style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 11, marginBottom: 7, overflow: "hidden" }}>
            <button onClick={() => setExpandedWeek(isOpen ? null : i)} style={{ display: "flex", alignItems: "center", gap: 9, width: "100%", padding: "12px 15px", background: "transparent", border: "none", cursor: "pointer", textAlign: "left", fontFamily: "'DM Sans',sans-serif" }}>
              ...
              <span style={{ flex: 1, fontSize: 13.5, fontWeight: 600, color: T.text, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{entry.label}</span>
  ...
  // RENDER
  // ─────────────────────────────────────────────
  const pages = { overview: <PageOverview />, checklist: <PageChecklist />, consider: <PageConsider />, board: <PageBoard />, impact: <PageImpact />, financials: <PageFinancials />, assets: <PageAssets />, weekly: <PageWeekly /> };
